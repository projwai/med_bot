...
COURSE_STRUCTURE = {
    ...
    ("3", "Musculoskeletal System"): {
        ...
        "doctors": {
            ...
            "Pathology":  ["د. فردوس إبراهيم", "د. عبدالله عبد القادر",
                           "د. يوسف", "د. الدسوقي إبراهيم"],
            "Anatomy":    ["د. أبوعلامة"],
            "DR":         [],
        }
    },
    ...
    ("4", "Cardiopulmonary 1"): {
        ...
        "doctors": {
            ...
            "DR":         ["د. أبوعلامة"],
            "Pathology":  ["د. يوسف عمر", "د. عبد العزيز خوجلي",
                           "د. فردوس إبراهيم", "د. عمر عدلان",
                           "د. ياسر محمد", "د. أحمد الريح", "د. محمد يوسف هجو"],
            "Physiology": ["د. محمد نور", "د. ناجي"],
        }
    },
    ...
    ("5", "Cardiopulmonary 2"): {
        ...
        "doctors": {
            ...
            "Clinical":              ["د. عبد الرحيم", "د. فردوس",
                                      "د. عبدالله عبد القادر", "د. منى مهنا",
                                      "د. أبومدين", "د. معز عبودي",
                                      "د. عبد العزيز خوجلي", "د. أحمد الريح"],
            "Examination + History": ["د. فردوس", "د. أحمد الريح",
                                      "د. منى مهنا", "د. عبد الرحيم",
                                      "د. عبدالله عبد القادر"],
        }
    },
    ("5", "Endocrinology"): {
        ...
        "doctors": {
            ...
            "Clinical":                         ["د. عبدالله عبد القادر",
                                                 "د. عبد الرحيم", "د. فردوس",
                                                 "د. منى مهنا", "د. أحمد الريح",
                                                 "د. عبد العزيز خوجلي",
                                                 "د. ياسر محمد", "د. أبومدين"],
            "Biochemistry":                     ["د. صلاح"],
            "Pathology":                        ["د. فردوس", "د. منى مهنا",
                                                 "د. عبدالله عبد القادر",
                                                 "د. عبد الرحيم", "د. أحمد الريح",
                                                 "د. أبومدين", "د. عبد العزيز خوجلي",
                                                 "د. ياسر محمد"],
        }
    },
    ("5", "Pharmacology"): {
    ...
    ("6", "Gastrointestinal Tract 2"): {
        ...
        "doctors": {
            ...
            "Clinical":   ["د. فردوس", "د. عبدالله عبد القادر",
                           "د. أحمد الريح", "د. منى مهنا",
                           "د. عبد الرحيم", "د. أبومدين"],
    ...
    ("7", "Genito-urinary System"): {
        ...
        "doctors": {
            ...
            "Clinical":              ["د. محمد الأمين", "د. ياسر البر",
                                      "د. المعز", "د. محمد المسلمي",
                                      "د. أحمد الريح"],
            "Examination + History": [],
        }
    },
    ("7", "Central Nervous System"): {
        ...
        "doctors": {
            ...
            "Clinical":              ["د. منى مهنا", "د. فردوس",
                                      "د. عبدالله عبد القادر", "د. أحمد الريح",
                                      "د. عبد الرحيم", "د. عبد العزيز",
                                      "د. أبومدين"],
...
def send_file(chat_id, file_id, content_type="pdf"):
    try:
        if content_type == "video":
            ...
            r = requests.post(f"{TELEGRAM_API}/sendAudio",
...
def get_courses_keyboard(semester):
    ...
    rows = [row[:] for row in courses_map.get(semester, [[{"text": "لا توجد مقررات"}]])]
...
async def webhook(update: dict, x_telegram_bot_api_secret_token: str = Header(None)):
    try:
        ...
        if file_info and is_admin(user) and chat_id in UPLOAD_SESSION:
            ...
            if session.get("type") == content_type:
                ...
                send_message(chat_id,
                    f"⚠️ نوع الملف غير متطابق! اخترت {session.get('type')} ولكن أرسلت {content_type}")
            return {"ok": True}
        ...
        if text == "❌ إلغاء العملية" and is_admin(user) and chat_id in UPLOAD_SESSION:
            UPLOAD_SESSION.pop(chat_id, None)
            send_message(chat_id, "❌ تم إلغاء عملية الرفع.",
                         reply_markup=get_main_keyboard(admin=True))
            return {"ok": True}
        ...
        if text and text.startswith("/addfile") and is_admin(user):
            ...
            if len(parts) >= 5:
                ...
                crud.add_material(semester, course, ctype, file_id,
                                  section=section, doctor=doctor)
                send_message(chat_id, "✅ تمت الإضافة بنجاح!")
            else:
                send_message(chat_id,
        ...
        if text == "ابدأ 🎓":
            USER_STATE.pop(chat_id, None)
            send_message(chat_id, "📚 اختر الفصل الدراسي:",
        ...
        if text == "⬅️ رجوع":
            ...
            # عند شاشة الدكتور → رجوع للأقسام
            if state.get("section") and state.get("course"):
                # نمسح section ونعرض الأقسام من جديد
                section_backup = state.pop("section")
            ...
            # عند شاشة القسم أو المقرر → رجوع للمقررات
            if state.get("course"):
        ...
        if text in semester_map:
            ...
            send_message(chat_id, "📖 اختر المقرر:",
                         reply_markup=get_courses_keyboard(semester))
            return {"ok": True}
        ...
        if text in course_names:
            ...
            if is_admin(user) and chat_id in UPLOAD_SESSION:
                session = UPLOAD_SESSION[chat_id]
                semester = session.get("semester")
                # إذا المقرر محدد بالفعل والنص قسم → تجاوز لمعالج القسم
                ...
                if state.get("course") and text in ALL_SECTIONS:
                    ...
                    if needs_section(semester, text):
                        sections = get_sections(semester, text)
                        send_message(chat_id, "📂 اختر القسم:",
                                     reply_markup=get_sections_keyboard(sections))
                    else:
                        send_message(chat_id, "🗂 اختر نوع المحتوى:",
                                     reply_markup=get_types_keyboard(short_label(text)))
                    return {"ok": True}
        ...
        if text in ALL_SECTIONS:
            ...
            if is_admin(user) and chat_id in UPLOAD_SESSION:
                ...
                logger.debug(f"[ADMIN SECTION] sem={semester} course={course} section={text} needs_doctor={needs_doctor(semester, course, text)}")
                if needs_doctor(semester, course, text):
                    doctors = get_doctors(semester, course, text)
                    send_message(chat_id, "👨‍⚕️ اختر الدكتور:",
            ...
            logger.debug(f"[USER SECTION] state={state} text={text} needs_doctor={needs_doctor(semester, course, text) if semester and course else 'N/A'}")
